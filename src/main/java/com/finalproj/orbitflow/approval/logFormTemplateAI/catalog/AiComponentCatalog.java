package com.finalproj.orbitflow.approval.logFormTemplateAI.catalog;
...
public class AiComponentCatalog {
    ...
    public static final AiComponentCatalogItem EVENT_DATE_RANGE =
            new AiComponentCatalogItem(
                    "event-date-range",
                    "일정",
                    "기간을 기반으로 캘린더 및 근태 도메인 이벤트를 생성하는 복합 일정 컴포넌트",
                    ...
                    List.of(
                            "휴가, 출장, 외근, 회사 일정 등 기간 기반 이벤트를 생성해야 할 때",
                            "문서 승인과 함께 캘린더 또는 근태 기록에 반영되어야 할 때"
                    ),

                    List.of(
                            "event-date-range 컴포넌트는 baseRole이 지정된 경우에만 사용할 수 있다",
                            "baseRole은 문서 작성 시작 시 이미 결정되어 있으며 AI가 변경하지 않는다",
                            "모든 경우에 시작일과 종료일은 필수 입력 항목이다",
                            "모든 경우에 일정 제목(title)과 설명(description)은 필수 입력 항목이다",
                            "입력된 제목과 설명은 캘린더 일정 및 근태 기록 생성에 직접 사용된다",
                            "기간, 제목, 설명을 다른 컴포넌트로 분리하지 않는다",
                            "event-date-range와 동일한 의미를 가지는 추가 입력 컴포넌트를 생성하지 않는다"
                    )
            );
